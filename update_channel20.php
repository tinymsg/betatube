...
<?php
    function clean($string) {
        ...
        $__user_u->update_row($_SESSION['siteusername'], "tags", $_GET['tags']);

        $__user_u->update_row($_SESSION['siteusername'], "msg", $_GET['title']);
		
		$_user2 = $__user_h->fetch_user_username($_SESSION['siteusername']);
		if($_user2['partner'] == "y") {
        if(!empty($_GET['channel_hidden'])) { 
            $__user_u->update_row($_SESSION['siteusername'], "hidden", $_GET['channel_hidden']);
        }
		}
		
        $__user_u->update_row($_SESSION['siteusername'], "redirect", $_POST['redir']);
		
        if(!empty($_POST['layout'])) { 
            $__user_u->update_row($_SESSION['siteusername'], "channelversion", $_POST['layout']);
        }
