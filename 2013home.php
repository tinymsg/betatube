										...
										<div class="context-data-container">
											<div class="lohp-newspaper-shelf">
												<div class="lohp-large-shelf-container">
                                                    <?php
                                                        $stmt = $__db->prepare("SELECT * FROM videos WHERE featured = 'v' AND visibility = 'n' ORDER BY id DESC LIMIT 1");
                                                        $stmt->execute();
												...
												<div class="lohp-medium-shelves-container">
                                                    <?php
                                                        $stmt = $__db->prepare("SELECT * FROM videos WHERE featured = 'v' AND visibility = 'n' ORDER BY id DESC LIMIT 1, 3");
                                                        $stmt->execute();
                                                        while($video = $stmt->fetch(PDO::FETCH_ASSOC)) {	
															$_user22 = $__user_h->fetch_user_username($video['author']);
                                                            $video['age'] = $__time_h->time_elapsed_string($video['publish']);		
												...
												<div>
													<div class="lohp-shelf-cell-container lohp-category-shelf last-shelf-in-line recent-videos">
														<div class="lohp-category-shelf-item-list lohp-shelf-size-3">
                                                            ...
                                                            <?php
                                                                $stmt = $__db->prepare("SELECT * FROM videos WHERE visibility = 'n' ORDER BY id DESC LIMIT 3");
                                                                $stmt->execute();
                                                                while($video = $stmt->fetch(PDO::FETCH_ASSOC)) {	
                                                                    $video['age'] = $__time_h->time_elapsed_string($video['publish']);		
                                                                    $video['duration'] = $__time_h->timestamp($video['duration']);
