		...
		<script>
			...
			function subscribe() {
				if(loggedIn == true) { 
					if(subscribed == false) { 
						$.ajax({
							...
							success: function(res) {
								...
								addAlert("editsuccess_" + alerts, "You are now subscribed to <?php echo htmlspecialchars($_user['username']); ?>");
						...
						$.ajax({
							...
							success: function(res) {
								...
								addAlert("editsuccess_" + alerts, "You are no longer subscribed to <?php echo htmlspecialchars($_user['username']); ?>");
								showAlert("#editsuccess_" + alerts);
								console.log("DEBUG: " + res);
								subscribed = false;
							}
					...
					alerts++;
					addAlert("editsuccess_" + alerts, "You must be logged in.");
					showAlert("#editsuccess_" + alerts);
				}
			}
		</script>
...
<?php header("Location: /user/" . $_GET['n']); ?>
<?php } else { ?>
<?php require($_SERVER['DOCUMENT_ROOT'] . "/s/mod/2013_discussion.php"); ?>
<?php } } ?>