...
class user_helper {
    ...
    function fetch_user_cid($username) {
        $stmt = $this->__db->prepare("SELECT * FROM users WHERE channelid = :username");
        $stmt->bindParam(":username", $username);
        $stmt->execute();
    ...
    function cid_exists($user) {
        $stmt = $this->__db->prepare("SELECT username FROM users WHERE channelid = :username");
        $stmt->bindParam(":username", $user);
        $stmt->execute();
    ...
    function fetch_plus($username) {
        $stmt = $this->__db->prepare("SELECT * FROM plus WHERE email = :username");
        $stmt->bindParam(":username", $username);
        $stmt->execute();
    ...
    function fetch_user_vanity($username) {
        $stmt = $this->__db->prepare("SELECT * FROM users WHERE vanity = :username");
        $stmt->bindParam(":username", $username);
        $stmt->execute();
    ...
    function fetch_uid($username) {
        $stmt = $this->__db->prepare("SELECT * FROM users WHERE id = :username");
        $stmt->bindParam(":username", $username);
        $stmt->execute();
    ...
    function vanity_exists($user) {
        $stmt = $this->__db->prepare("SELECT username FROM users WHERE vanity = :username");
        $stmt->bindParam(":username", $user);
        $stmt->execute();
    ...
    function fetch_subs_count($username) {
        ...
        return number_format($stmt->rowCount());
    }
