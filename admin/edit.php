				...
				<div id="page" class="  home     branded-page-v2-detached-top  clearfix">
					...
					<div id="browse-main-column" style="float: left;margin: 14px 0 0 25px;" class="ytg-4col">
						<div class="browse-collection  has-box-ad">
								...
								<hr>
								<?php $user = $__user_h->fetch_user_username($_GET['u']); ?>
						...
						<strong style="display:block;margin:0 0 5px"><?php echo htmlspecialchars($user['username']) ?></strong><form action="edituser">
							Joined: <?php echo date("M d, Y", strtotime($user['created'])); ?><br>
							Last active: <?php echo date("M d, Y", strtotime($user['lastlogin'])); ?><br>
							Subs: <?php echo $__user_h->fetch_subs_count($user['username']); ?><br>
							Title: <input class="yt-uix-form-input-text" name="title" style="margin-bottom:5px;" placeholder="Type title here..." value="<?php if($user['title'])	{	?><?php echo htmlspecialchars($user['title']); ?><?php } else{ ?><?php echo htmlspecialchars($user['username']); ?><?php } ?>"></input><br>
							Bio: <input class="yt-uix-form-input-text" name="bio" style="margin-bottom:5px;" placeholder="Type bio here..." value="<?php echo htmlspecialchars($user['bio']); ?>"></input><br>
							Country: <input class="yt-uix-form-input-text" name="country" style="margin-bottom:5px;" placeholder="Type country here..." value="<?php echo htmlspecialchars($user['country']); ?>"></input><br>
							Featured Channels: <input class="yt-uix-form-input-text" name="featured_channels" style="margin-bottom:5px;" placeholder="Type channels here..." value="<?php echo htmlspecialchars($user['featured_channels']); ?>"></input><br>
							Website: <input class="yt-uix-form-input-text" name="website" style="margin-bottom:5px;" placeholder="Type website here..." value="<?php echo htmlspecialchars($user['website']); ?>"></input><br>
							Layout: <input class="yt-uix-form-input-text" name="channelversion" style="margin-bottom:5px;" placeholder="Type layout here..." value="<?php if($user['channelversion'] != '20') { ?><?php echo htmlspecialchars($user['channelversion']); ?><?php } ?>"></input>
							<br><button type="submit" name="user" style="padding:10px 4px" value="<?php echo htmlspecialchars($user['username']) ?>">Update</button>
						</div></form>
						<div style="float:right">
						<form action="/d/admin?action=actually_just_ban" method="POST" onsubmit="return confirm('Are you sure you want to ban this user?');">
						<button type="submit" name="users" style="padding:34px 4px" value="<?php echo htmlspecialchars($user['username']) ?>">Ban User</button>
						</form>
						<form action="edituser?m=p" method="POST" onsubmit="return confirm('Are you sure you want to partner this user?');">
						<button type="submit" name="user" style="padding:34px 4px" value="<?php echo htmlspecialchars($user['username']) ?>">Partner User</button>
						</form>
						</div>
						</div>
						</div>
