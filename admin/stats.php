			...
			<div id="page-container">
				<div id="page" class="  home     branded-page-v2-detached-top  clearfix">
					...
					<?php 
								...
								$stmt = $__db->prepare("SELECT id FROM `users` WHERE channelversion = 'cosmic'");
								$stmt->execute();
								$userscosmic = $stmt->rowCount();
								
								$stmt = $__db->prepare("SELECT id FROM `users` WHERE channelversion = 'one'");
								$stmt->execute();
								$usersone = $stmt->rowCount();
								
								$stmt = $__db->prepare("SELECT id FROM `users` WHERE partner = 'y'");
								$stmt->execute();
								$userspart = $stmt->rowCount();
						...
						<?php $server = get_server_array_stats(); ?>
							...
							Users registered: <b style="float:right;"><?php echo $users; ?></b><br>
							Partners: <b style="float:right;"><?php echo $userspart; ?></b><br>
						...
						<?php $server = get_server_array_stats(); ?>
							Cosmic Panda Channels: <b style="float:right;"><?php echo $userscosmic; ?></b><br>
							One channel Channels: <b style="float:right;"><?php echo $usersone; ?></b><br>
